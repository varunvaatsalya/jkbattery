...
function Customers() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await window.electron.invoke("database-operation", {
      action: id ? "update-customer" : "save-customer",
      data,
    });
  ...
  return (
    <div className="w-full flex flex-col items-center h-screen">
      ...
      <div className="mb-1 w-full md:w-4/5 lg:w-3/5 text-center font-semibold text-white bg-gray-800 rounded-xl py-1 px-2 flex items-center">
        ...
        <div className="w-[15%] text-start">#CID</div>
      ...
      <ul className="flex-1 w-full md:w-4/5 lg:w-3/5 bg-gray-200 rounded-xl p-2 space-y-1 overflow-y-auto scrollbar-hide">
        {customers.map((customer, index) => (
          ...
          >
            ...
            <div className="w-[15%]">{customer.eid}</div>
            <div className="w-[25%]">{customer.name}</div>
            <div className={"w-[15%] text-rose-600"}>{customer.contact}</div>
            <div className="w-[35%] line-clamp-1" title={customer.address}>{customer.address}</div>
            {customer.id !== 0 && (
              <div
